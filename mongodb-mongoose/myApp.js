...
var createAndSavePerson = (done) => {
  ...
  let sena = new Person({name:"Sena", age:29, favoriteFoods:['Brokoli', 'Lettuce']});
...
let arrayOfPeople = [
  {name: "Erkan", age: 31, favoriteFoods: ["Lentil"]},
  {name: "Musa", age: 32, favoriteFoods: ["Brokoli"]}
];
...
var findOneByFood = (food, done) => {
  ...
  Person.findOne({favoriteFoods: food}, (err, foodFind) => {
    if(err) return console.log(err);
    done(null, foodFind);
  })
};
...
var findPersonById = (personId, done) => {
  ...
  Person.findById({_id: personId}, (err, idFind) => {
    if(err) return console.log(err);
    done(null, idFind);
  }) 
};
...
const queryChain = (done) => {
  ...
  Person.find({favoriteFoods: {$all: [foodToSearch]}})
    ...
    .limit(2)
    .select('-age')
    .exec((err, filteredByFood) => {
      if(err) return console.log(err);
      done(null, filteredByFood)
    })
};
